...
main =
    ...
    it "brute-force" $ do
        ...
        flip finally showLatest $
            forConcurrently_ (zip [0 ..] packages) $ \(i, GetPackage pname) -> do
                waitQSem qs
                revs <-
                    getJson $
