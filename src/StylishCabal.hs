...
module StylishCabal (prettify, FormatOpts(..)) where
...
prettify input f = (`runReaderT` f) $ do
    gpd <- do
...
renderSetupBuildInfo SetupBuildInfo{..} = mkBlock
    (string "custom-setup")
...
renderSourceRepo SourceRepo{..} = mkBlock
...
renderPackageDesc pd@PackageDescription{..} = sequence
...
renderNodes :: Traversable t
            => (a -> BuildInfo)
            -> (a -> StyleM [Thing])
            -> t (CondBranch ConfVar c a)
            -> StyleM [Thing]
renderNodes f renderMore branches = do
    subnodes <- mapM (renderCondNode f renderMore) branches
    return $ concat subnodes
...
renderTest testName CondNode{..} = Block
    ...
    <$> (do
        cd <- renderTestData condTreeData
...
renderCondNode getBuildInfo extra (CondBranch pred' branch1 branch2) = do
    b1 <- Block (string "if" <+> showPredicate pred')
        <$> (do
            b <- extra (condTreeData branch1)
            bi <- showBuildInfo (getBuildInfo $ condTreeData branch1)
            ns <- renderNodes getBuildInfo extra (condTreeComponents branch1)
            return $ b ++ bi ++ ns)
    b2 <- case branch2 of
        Nothing -> pure Nothing
        Just b2 -> fmap Just $ Block (string "else")
            <$> (do
                b <- extra (condTreeData b2)
