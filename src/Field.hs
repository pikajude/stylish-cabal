...
instance Show (Int -> Doc -> Doc) where
    show _ = "<fn>"
...
mkNonempty :: (Foldable t, Monad m) => String -> t a -> (t a -> Doc) -> m Thing
...
renderFields blkFields = fmap (vcat . catMaybes) (mapM (renderField width') blkFields)
  where
