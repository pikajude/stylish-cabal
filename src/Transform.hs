...
toBlocks GenericPackageDescription {..} =
  ...
  , concat
    ...
    , map (uncurry (libToBlock packageDescription) . first Just)
          condSubLibraries
...
pdToFields pd@PackageDescription {..} =
  [ guard newSpec >> cabalVersion "cabal-version" specVersionRaw
    ...
    , desc description
    , either (spdxLicenseField "license")
...
libToBlock pkg libname CondNode {..} = deepseq condTreeConstraints $ Block
...
libDataToFields Library {..} =
  libName
    `deepseq` [ ffilter not (stringField "exposed" . show) libExposed
...
foreignLibToBlock pkg libname CondNode {..} =
  condTreeConstraints `deepseq` Block
...
foreignLibDataToFields ForeignLib {..} =
  foreignLibName
    `seq` [ ffilter (== ForeignLibNativeShared) (flibType "type") foreignLibType
...
exeToBlock pkg exeName CondNode {..} = deepseq condTreeConstraints $ Block
...
exeDataToFields Executable {..} =
  exeName
    `seq` [ nonEmpty (stringField "main-is") modulePath
          , ffilter (== ExecutablePrivate)
                    (\_ -> stringField "scope" "private")
                    exeScope
          ]

testToBlock pkg testName CondNode {..} = deepseq condTreeConstraints $ Block
...
testDataToFields TestSuite {..} = testName `seq` case testInterface of
  TestSuiteExeV10 v f ->
...
benchToBlock pkg benchName CondNode {..} = deepseq condTreeConstraints $ Block
...
benchDataToFields Benchmark {..} =
  benchmarkName `seq` case benchmarkInterface of
    BenchmarkExeV10 v f ->
      v `seq` [stringField "type" "exitcode-stdio-1.0", stringField "main-is" f]
    _ -> []
...
condNodeToBlock pkg getBuildInfo extra (CondBranch pred' branch1 branch2) =
  let b1 = deepseq (condTreeConstraints branch1) $ Block
        (If pred')
      ...
      b2 = branch2 <&> \b -> deepseq (condTreeConstraints b) $ Block
        Else
...
buildInfoToFields _ BuildInfo {..} =
  staticOptions
    `seq` -- staticOptions isn't in the parser yet
  ...
  where (oldTools, newTools) = (buildTools, buildToolDepends)
